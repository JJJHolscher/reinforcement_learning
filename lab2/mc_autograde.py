...
class SimpleBlackjackPolicy(object):
    ...
    def get_probs(self, states, actions):
        ...
        probs = []
        for state in states:
            sample = self.sample_action(state)
            prob = np.array([sample == action for action in actions])
            probs.append(prob.astype(float))
...
def mc_prediction(env, policy, num_episodes, discount_factor=1.0, sampling_function=sample_episode):
    ...
    for _ in range(num_episodes):
        for states, actions, rewards, dones in sampling_function(env, policy):
            print(states)
